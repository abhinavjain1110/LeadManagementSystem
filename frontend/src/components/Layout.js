...
const Layout = ({ children }) => {
  ...
  const isActive = (href) => {
    ...
    if (href === '/leads/new' && location.pathname === '/leads/new') return true;
    if (href === '/leads/:id/edit' && location.pathname.includes('/leads/') && location.pathname.includes('/edit')) return true;
    return false;
  };
      ...
      <div className={`fixed inset-0 z-50 lg:hidden ${sidebarOpen ? 'block' : 'hidden'}`}>
        ...
        <div className="fixed inset-y-0 left-0 flex w-64 flex-col bg-white">
          <div className="flex h-16 items-center justify-between px-4">
            <h1 className="text-xl font-semibold text-gray-900">Erino</h1>
            <button
